...
function loadFromStorage() {
  ...
  if (currentTheme === "light") {
    ...
    // ИСПРАВЛЕНА КРИТИЧЕСКАЯ ОПЕЧАТКА: removeCN заменено на стандартный classList.remove
    document.documentElement.classList.remove("light-theme");
    if (btn)
      btn.innerHTML = `<svg enable-background="new 0 0 24 24" height="24" viewBox="0 0 24 24" width="24" focusable="false" class="icon-theme"><rect fill="none" height="24" width="24"></rect><path d="M9.37,5.51C9.19,6.15,9.1,6.82,9.1,7.5c0,4.08,3.32,7.4,7.4,7.4c0.68,0,1.35-0.09,1.99-0.27C17.45,17.19,14.93,19,12,19 c-3.86,0-7-3.14-7-7C5,9.07,6.81,6.55,9.37,5.51z M12,3c-4.97,0-9,4.03-9,9s4.03,9,9,9s9-4.03,9-9c0-0.46-0.04-0.92-0.1-1.36 c-0.98,1.37-2.58,2.26-4.4,2.26c-2.98,0-5.4-2.42-5.4-5.4c0-1.81,0.89-3.42,2.26-4.4C12.92,3.04,12.46,3,12,3L12,3z"></path></svg>`;
  }
  ...
  // РЕАКТИВНЫЙ СКИННИНГ: Принудительно схлопываем контейнер при старте, если флаг равен true
  const isTerminalSavedCollapsed =
    localStorage.getItem("bybit_terminal_collapsed") === "true";
  const terminalContainer = document.getElementById("main-terminal-container");
  const icon = document.getElementById("collapse-svg-icon");
  if (isTerminalSavedCollapsed && terminalContainer) {
    terminalContainer.classList.add("collapsed-terminal");
...
function restoreTabsVisualOnly() {
  ...
  if (targetValueSpan && targetValueSpan.parentNode) {
    const labelSpan = targetValueSpan.parentNode.querySelector(".an-label");
    if (labelSpan) {
      labelSpan.innerText = "РИСК";
    }
  }
...
function calculate() {
  ...
  // Пытаемся получить Стоп-Лосс из интерфейса, если у тебя есть такое поле ввода.
  // Если поля на экране нет (targetSL станет 0) — калькулятор автоматически рассчитает его по волатильности ATR.
  const slInput = document.getElementById("sl-price");
  let targetSL = slInput ? parseFloat(slInput.value) : 0;
  ...
  // Автоматический расчет Стоп-Лосса на основе волатильности, если трейдер не ввел его вручную
  if (!targetSL || targetSL <= 0) {
    if (currentSide === "Long") {
      targetSL = entryPrice * (1 - marketVolatility / leverage);
    } else {
      targetSL = entryPrice * (1 + marketVolatility / leverage);
    }
  }

  // 🎯 РАСЧЕТ РИСКА В ДОЛЛАРАХ (Железно 2% от депозита = 2 USDT)
  ...
  /* === КОНЕЦ ЧАСТИ 8 === */
  /* === НАЧАЛО ЧАСТИ 9 === */
  // Процент изменения цены от входа до стопа (как коэффициент)
  const priceMovePercentSL = Math.abs(entryPrice - sl) / entryPrice;

  if (currentTab === "futures") {
    // Включаем базовые комиссии Bybit Taker (0.055% на вход и 0.055% на выход) в расчет риска
    const totalFeeRate = TAKER_FEE * 2;

    // МАТЕМАТИЧЕСКИ ПРАВИЛЬНЫЙ ОБЪЕМ (Value) ПОД ЗАДАННЫЙ РИСК
    totalVolume = riskAmount / (priceMovePercentSL + totalFeeRate);

    // Требуемая изолированная маржа под этот объем
    cost = totalVolume / leverage;
    qty = totalVolume / entryPrice;

    if (currentSide === "Long") {
      // Честный Тейк-Профит с учетом соотношения сторон (PR ratio)
      tp = entryPrice * (1 + priceMovePercentSL * rewardMultiplier);
    ...
    // Расчет чистых результатов сделки для вывода в журнал и информеры Bybit (с учетом комиссий)
    bybitRawProfit =
      Math.abs(tp - entryPrice) * qty - totalVolume * TAKER_FEE * 2;
    bybitRawLoss =
      Math.abs(entryPrice - sl) * qty + totalVolume * TAKER_FEE * 2;
    bybitRoiTP = (bybitRawProfit / cost) * 100;
    bybitRoiSL = (bybitRawLoss / cost) * 100;
  } else {
    // Спотовая логика
    const spotFee = 0.001;
    totalVolume = riskAmount / (priceMovePercentSL + spotFee * 2);
    cost = totalVolume; // На споте маржа = объему
    leverage = 1;
    qty = totalVolume / entryPrice;
    tp = entryPrice * (1 + priceMovePercentSL * rewardMultiplier);
    liq = "—";
    ...
    bybitRawProfit =
      Math.abs(tp - entryPrice) * qty - totalVolume * spotFee * 2;
    bybitRawLoss = Math.abs(entryPrice - sl) * qty + totalVolume * spotFee * 2;
    bybitRoiTP = (bybitRawProfit / cost) * 100;
    bybitRoiSL = (bybitRawLoss / cost) * 100;
  }

  // Переводим изменения цен в привычные для твоего UI проценты (например, 2.50%)
  pctChangeSL = ((sl - entryPrice) / entryPrice) * 100;
  pctChangeTP = ((tp - entryPrice) / entryPrice) * 100;

  // Восстанавливаем расчет запаса свободной маржи из твоей старой Части 8
  const freeMargin = balance - cost;
  const remainingTrades = cost > 0 ? Math.floor(freeMargin / cost) : 0;

  document.getElementById("res-margin-free").innerText =
    `$${Math.max(0, freeMargin).toFixed(2)}`;
  document.getElementById("margin-trades").innerText =
    `Запас на ${Math.max(0, remainingTrades)} сделки`;
...
// ФУНКЦИЯ РЕАКТИВНОГО СВОРАЧИВАНИЯ БЕЗ СКАЧКОВ И С ЖЕСТКИМ КЛИНИНГОМ ИНЛАЙН-СТИЛЕЙ
function toggleTerminalCollapse() {
  document.documentElement.classList.remove("init-terminal-hidden");
  const terminalContainer = document.getElementById("main-terminal-container");
  const icon = document.getElementById("collapse-svg-icon");
  const textLabel = document.getElementById("collapse-text-indicator");
  if (!terminalContainer) return;

  // Возвращаем плавность анимации перед началом переключения
  terminalContainer.style.transition = "";

  // Проверяем, свернут ли блок прямо сейчас
  const isCurrentlyCollapsed =
    terminalContainer.classList.contains("collapsed-terminal");

  if (!isCurrentlyCollapsed) {
    // Сворачиваем блок: класс в CSS применит max-height: 0px плавным переходом
    terminalContainer.classList.add("collapsed-terminal");
    if (icon) icon.style.transform = "rotate(180deg)";
    if (textLabel) textLabel.innerText = "ПОКАЗАТЬ ОСНОВНОЙ БЛОК";
    localStorage.setItem("bybit_terminal_collapsed", "true");
  } else {
    // Разворачиваем блок: полностью очищаем инлайновые стили прелоадера, чтобы запустить CSS-анимацию
    terminalContainer.style.maxHeight = "";
    terminalContainer.style.minHeight = "";
    terminalContainer.style.paddingTop = "";
    terminalContainer.style.paddingBottom = "";
    terminalContainer.style.marginBottom = "";
    terminalContainer.style.borderWidth = "";
    terminalContainer.style.boxShadow = "";

    terminalContainer.classList.remove("collapsed-terminal");
    if (icon) icon.style.transform = "rotate(0deg)";
    if (textLabel) textLabel.innerText = "СКРЫТЬ ОСНОВНОЙ БЛОК";
    localStorage.setItem("bybit_terminal_collapsed", "false");

    if (typeof calculate === "function") calculate();
  }
}

// СИНХРОНИЗАТОР ТЕКСТА КНОПКИ: Срабатывает сразу, как только сформируется структура документа
(function () {
  window.addEventListener("DOMContentLoaded", function () {
    const isCollapsed =
      localStorage.getItem("bybit_terminal_collapsed") === "true";
    const textLabel = document.getElementById("collapse-text-indicator");
    if (isCollapsed && textLabel) {
      textLabel.innerText = "ПОКАЗАТЬ ОСНОВНОЙ БЛОК";
    }
  });
})();
/* === КОНЕЦ ЧАСТИ 18 === */
